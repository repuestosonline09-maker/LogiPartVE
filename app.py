...
# 1. Configuración de página (Protegida)
...
# 2. CSS Ultra-Compacto (Mantenido v4.7/5.3)
st.markdown("""
    <style>
...
with c_logo2:
    st.title("LogiPartVE: Gestión Experta y Alertas Críticas")
...
# 6. Lógica de IA - CALIBRADA (Resumida en técnica, extensa en noticias)
if st.button("🚀 GENERAR ANÁLISIS Y COTIZACIÓN", type="primary"):
    if v_in and r_in and n_in:
        try:
            ...
            prompt = f"""
            ACTÚA COMO EXPERTO SENIOR EN RECAMBIOS Y LOGÍSTICA. 
            
            1. ANÁLISIS TÉCNICO (RESUMIDO): Identifica {n_in} para {r_in} ({v_in}). 
               - Menciona brevemente sustitutos actuales.
               - USA TU CONOCIMIENTO de pesos y medidas originales. Solo detente si el número es inválido.

            2. COTIZACIÓN DDP VENEZUELA (RESUMIDA):
               - Muestra Peso Físico, Dimensiones y Peso a Facturar (Mayor entre Físico y Volumétrico + 20%).
            ...
            3. MONITOREO DE NOTICIAS Y ALERTAS (EXTENSO Y DETALLADO):
               - Análisis profundo de noticias de HOY (Diciembre 2025) que afecten la ruta {o_in} a Venezuela (Clima, Aduanas, Puertos, retrasos regionales).

            FORMATO: Lista corta para puntos 1 y 2. Párrafos detallados para punto 3.
            """

            with st.spinner('Analizando y monitoreando rutas...'):
...
# 8. TABLA MANUAL (Mantenida sin cambios)
