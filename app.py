...
# --- LÓGICA DE DISEÑO RESPONSIVE (MÓVIL VS PC) ---
st.markdown(
    """
    <style>
    /* Ajuste para móviles */
...
# --- DISEÑO DEL PANEL CENTRAL (LOGO INTELIGENTE) ---
c_left, c_logo, c_right = st.columns([1.5, 1, 1.5])
with c_logo:
    if os.path.exists(logo_filename):
        # El logo se ajusta solo gracias al CSS de arriba
        st.markdown(f'<div class="main-logo-container"><img src="data:image/png;base64,{base64.b64encode(open(logo_filename, "rb").read()).decode()}" style="width:100%"></div>', unsafe_allow_html=True)
    else:
...
if st.button("🚀 GENERAR ANÁLISIS Y COTIZACIÓN PROFESIONAL", type="primary", use_container_width=
